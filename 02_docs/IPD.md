...
## 13. 단계별 로드맵
1. 킥오프: 폴더/requirements 세팅, SMA 크로스 예제 JSON.
2. 데이터 확보: yfinance 파이프라인, pykrx 보조 설계.
3. 전략 JSON/룰엔진: 스키마와 파서 구현.
4. 프로토타입 엔진: backtesting.py 검증.
5. 프로덕션 엔진: Backtrader 전환, 수수료/슬리피지 반영.
6. 확장: VectorBT로 대량 파라미터 실험, 전략군 확대.
